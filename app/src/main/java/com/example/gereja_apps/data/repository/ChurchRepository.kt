...
object ChurchRepository {
    ...
    suspend fun getCategories() = runCatching { masterApi.getCategories().data ?: emptyList() }
    
    suspend fun getArticles(page: Int = 1) = runCatching { masterApi.getArticles(page).data?.data ?: emptyList() }
    
    suspend fun getChurches(search: String? = null, categoryId: Int? = null) = 
        runCatching { churchApi.getChurches(search, categoryId).data?.data ?: emptyList() }
        
    suspend fun getNearbyChurches(lat: Double, lng: Double, search: String? = null, categoryId: Int? = null) = 
        runCatching { churchApi.getNearbyChurches(lat, lng, search, categoryId).data?.data ?: emptyList() }
        
    suspend fun getChurchDetail(slug: String) = 
        runCatching { churchApi.getChurchDetail(slug).data }
}
